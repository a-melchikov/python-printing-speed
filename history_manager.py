...
class HistoryManager:
	...
	def save_result(self, count_word, language, frequency, error_count, total_chars, accuracy, elapsed_time, speed):
		logger.info(f"Сохранение результата в файл: {self.file_path}")
		with open(self.file_path, "a", encoding="utf-8") as f:
			f.write(
				f"{language}{frequency}({count_word}) {error_count}/{total_chars} {accuracy:.2f} {elapsed_time:.2f} {speed:.2f} "
				f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
			logger.info(
				f"В файл {self.file_path} успешно сохранены данные: {language}{frequency}({count_word}) "
				f"{count_word} {error_count}/{total_chars} "
